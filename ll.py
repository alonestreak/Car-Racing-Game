...
count=3
display_height=600
crash_sound=pygame.mixer.Sound("oggcrash.ogg")
pygame.mixer.music.load("oggback.ogg")
#stone=pygame_function.makeSprite("stone.jpg")
...
intro_background=pygame.image.load("newback.jpg")
...
def intro_loop():
    ...
    while intro:
        ...
        text=largetext.render("CAR GAME",True,snow_white)
        gamedisplays.blit(text, TextRect,)

        smalltext = pygame.font.Font('freesansbold.ttf', 20,)
        TextSurf, TextRect = text_objects("Created By - ", smalltext)
        TextRect= (120, 180)
        text =smalltext .render("Created By -> ", True, snow_white)
        gamedisplays.blit(text, TextRect)

        TextSurf, TextRect = text_objects("112 : Sushant Sawant", smalltext)
        TextRect= (150, 220)
        text =smalltext .render("112 : Sushant Sawant", True, snow_white)
        gamedisplays.blit(text, TextRect)

        button("START",150,520,100,50,snow_white,bright_green,"play")
        button("QUIT",550,520,100,50,snow_white,red,"quit")
        button("INSTRUCTION",300,520,200,50,snow_white,bright_blue,"intro")
...
def button(msg,x,y,w,h,ic,ac,action=None):
    ...
    if x+w>mouse[0]>x and y+h>mouse[1]>y:
        pygame.draw.ellipse(gamedisplays,ac,(x,y,w,h))
...
def introduction():
    ...
    while introduction:
        ...
        ptextSurf,ptextRect=text_objects("",smalltext)
...
def obstacle(obs_startx,obs_starty,obs):
    if obs==0:
        obs_pic=pygame.image.load("newrock.jpg")
    elif obs==1:
        obs_pic=pygame.image.load("car5.jpg")
    elif obs==2:
        obs_pic=pygame.image.load("tyre.jpg")
    elif obs==3:
        obs_pic=pygame.image.load("car7.jpg")
    elif obs==4:
        obs_pic=pygame.image.load("tree.jpg")
    elif obs==5:
        obs_pic=pygame.image.load("car6.jpg")
    elif obs==6:
        obs_pic=pygame.image.load("21.jpg")
    
    gamedisplays.blit(obs_pic,(obs_startx,obs_starty))

def score_system(passed,score,count):
    font=pygame.font.SysFont(None,25)
    text=font.render("PASSED="+str(passed),True,black)
    score=font.render("SCORE="+str(score),True,black)
    live=font.render("LIVES="+str(count),True,red)
    gamedisplays.blit(text,(0,50))
    gamedisplays.blit(score,(0,30))
    gamedisplays.blit(live,(0,70))
...
def message_display(text,passed,score,level):
    largetext=pygame.font.Font("freesansbold.ttf",80)
    ...
    time.sleep(3)
    game_loop(passed,score,level)


def message_displayy(text,passed,score,level):
    largetext=pygame.font.Font("freesansbold.ttf",80)
    textsurf,textrect=text_objects(text,largetext)
    ...
    pygame.display.update()
    largetext=pygame.font.Font("freesansbold.ttf",80)
    textsurff,textrectt=text_objects("NEW GAME",largetext)
    textrectt.center=((display_width/2),(display_height/2))
    gamedisplays.blit(textsurff,textrectt)
    pygame.display.update()
    time.sleep(2)
    game_loop()


def crash(passed,score,level):
    pygame.mixer.music.stop()
    pygame.mixer.Sound.play(crash_sound)
    message_display("YOU NOOB!!!!",passed,score,level)

def crashh():
    pygame.mixer.music.stop()
    pygame.mixer.Sound.play(crash_sound)
    message_display("YOU NOOB!!!",passed,score)
...
def game_loop(passed=0,score=0,level=0):
    global count
    
    if passed !=0 and score !=0:
        pass
    else:
        passed=0
        score=0
    ...
    while count>0:
        ...
        while not bumped:
            ...
            car(x,y)
            score_system(passed,score,count)
            if (x>690-car_width or x<110) or (x>display_width-(car_width+110) or x<110):
                count=count-1
                if count ==0:
                    pass
                else:
                    crash(passed,score,level)
            ...
            if count ==0:
                ...
                passed=0
                level=0
                message_displayy("GAME OVER",passed,score,level)
                pygame.display.update()
                count=3
                break
            button("Pause",650,0,150,50,blue,bright_blue,"pause")
